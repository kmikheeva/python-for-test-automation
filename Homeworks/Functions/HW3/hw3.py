...
def custom_range(sequence, *args) -> List[str]:
    if len(args) == 1:
        return list(sequence[:sequence.index(args[0])])
    elif len(args) == 2:
        return list(sequence[sequence.index(args[0]):sequence.index(args[1])])
    else:
        return list(sequence[sequence.index(args[0]):sequence.index(args[1]):args[2]])
